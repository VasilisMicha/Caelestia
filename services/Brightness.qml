...
Singleton {
  ...
  component Monitor: QtObject {
    ...
    readonly property Process initProc: Process {
      stdout: StdioCollector {
        onStreamFinished: {
          // Sync with your hyprsunset temp file
    ...
    function setBrightness(value: real): void {
      ...
      // Execute hyprsunset directly and update the temp file
      Quickshell.execDetached(["sh", "-c", `hyprctl hyprsunset gamma ${rounded} && echo ${rounded} > /tmp/hyprsunset_bright`]);
    }

    function initBrightness(): void {
      // Read from your temp file so Quickshell knows the current state on startup
      initProc.command = ["cat", "/tmp/hyprsunset_bright"];
