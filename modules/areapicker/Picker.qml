    ...
    function save(): void {
        ...
        CUtils.saveItem(screencopy, tmpfile, Qt.rect(Math.ceil(rsx), Math.ceil(rsy), Math.floor(sw), Math.floor(sh)), path => {
            ...
            Quickshell.execDetached(["sh", "-c",
                `ACTION=$(notify-send -a "caelestia-cli" -i "${path}" "Screenshot taken" "Copied to clipboard." --action="default=Save Screenshot" --wait); if [ "$ACTION" = "default" ]; then ~/Documents/Projects/Scripts/screenshot_save_dialog.sh "${path}"; fi`
            ]);
            closeAnim.start();
        });
    }
