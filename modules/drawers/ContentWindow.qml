...
StyledWindow {
    ...
    readonly property bool isLaptop: screen.name === "eDP-1"
    ...
    property real fsTransitionProg: (hasFullscreen || !visibilities.bar) ? 1 : 0
    ...
    readonly property real borderLayoutThickness: (hasFullscreen || !visibilities.bar) ? 0 : contentItem.Config.border.thickness
    ...
    Region {
        ...
        Region {
            x: root.width - width
            y: panels.osd.y + root.borderThickness
            // If it's the laptop, use the calculated width, otherwise 0
            width: root.isLaptop ? (panels.osd.width * (1 - panels.osd.offsetScale) + root.borderThickness) : 0
            height: root.isLaptop ? panels.osd.height : 0
        }
    }
    ...
    Item {
        ...
        BlobGroup {
            ...
            color: Qt.tint(Colours.palette.m3surface, Qt.alpha(Colours.palette.m3primaryContainer, 0.18))
        ...
        PanelBg {
            ...
            panel: panels.osd
            deformAmount: 0.25
            x: panels.osd.x + bar.implicitWidth
            // Hide the background if not laptop
            implicitWidth: root.isLaptop ? panels.osd.width : 0
            implicitHeight: root.isLaptop ? panel.height : 0
        }
