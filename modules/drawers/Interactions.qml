...
CustomMouseArea {
    ...
    onContainsMouseChanged: {
        if (!containsMouse && !Visibilities.pickerActive) {
    ...
    onPositionChanged: event => {
        ...
        if (!visibilities.bar && Config.bar.showOnHover && screen.name !== "eDP-1" && x < bar.clampedWidth)
            bar.isHovered = true;
        ...
        if (panels.sidebar.offsetScale === 1) {
            ...
            root.panels.osd.hovered = inOsd;
            if (!inOsd && osdShortcutActive) {
            ...
            // Show osd on hover
            const inOsd = outOfSidebar && inRightPanel(panels.osd, x, y);
            root.panels.osd.hovered = inOsd;
            if (!inOsd && osdShortcutActive) {
                osdShortcutActive = false;
            }
        ...
        if (utilitiesShortcutActive) {
            const showUtilities = Config.utilities.enabled && inBottomPanel(panels.utilities, x, y, true) && x > width - panels.utilities.width - Config.border.rounding;
            if (showUtilities) {
